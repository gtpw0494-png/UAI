...
assert.ok(caps25.length>=42);assert.equal(caps25.filter(x=>x.availability==="CONNECTED").length,35);assert.ok(caps25.some(x=>x.id==="research.openai.gpt_oss"&&x.availability==="CONNECTED"));assert.ok(!caps25.some(x=>x.id==="external.openai.api"));for(const id of ["local.forgelm.embeddings","local.forgelm.rerank","local.forgelm.long_context"])assert.ok(caps25.some(x=>x.id===id&&x.availability==="UNAVAILABLE"));
