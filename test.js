...
const v17chat=new OneChatRouter({research,development,explorative,tasks,knowledge,agents:migrated,store,audit,forgelm:{status:async()=>({state:"SUCCESS",checkpointExists:true}),chat:async()=>({state:"SUCCESS",text:"local model output"}),train:async()=>({state:"SUCCESS"})},learning:{build:()=>({state:"SUCCESS"}),file:"x"},selfdev:{stage:()=>({state:"SUCCESS"}),promote:()=>({state:"SUCCESS"})},control,sourceRegistry:sourceRegistryV12,dependencyStatus:()=>({state:"SUCCESS",dependencies:{torch:"CONNECTED"}}),modelLab:labStub});
...
const trainTurn=await v17chat.handle({message:"train ForgeLM 5 steps preset termux-tiny"});assert.equal(trainTurn.state,"SUCCESS");assert.ok(trainTurn.contributions.some(x=>x.agent==="forgelm"&&x.result.stepsThisRun===5));
...
const v18chat=new OneChatRouter({research,development,explorative,tasks,knowledge,agents:migrated,store,audit,forgelm:{status:async()=>({state:"SUCCESS",checkpointExists:true}),chat:async()=>({state:"SUCCESS",text:"x"})},learning:{build:()=>({state:"SUCCESS"}),file:"x"},selfdev:{},control,sourceRegistry:sourceRegistryV12,dependencyStatus:()=>({state:"SUCCESS",dependencies:{torch:"CONNECTED"}}),modelLab:tokLab});
const tokTurn=await v18chat.handle({message:"train tokenizer 320"});assert.equal(tokTurn.state,"SUCCESS");assert.ok(tokTurn.contributions.some(x=>x.agent==="forgelm"&&x.result.vocabSize===320));console.log("v0.18.0 tokenizer routing tests passed");
...
const helloTurn=await responseChat.handle({message:"hello"});assert.match(helloTurn.message,/OneChat is responding correctly/i);assert.equal(helloTurn.responseMode,"governed-composer");
...
assert.equal(caps25.length,42);assert.equal(caps25.filter(x=>x.availability==="CONNECTED").length,35);assert.ok(caps25.some(x=>x.id==="research.openai.gpt_oss"&&x.availability==="CONNECTED"));assert.ok(!caps25.some(x=>x.id==="external.openai.api"));
...
assert.equal(fullyLocal25.filter(x=>x.availability==="CONNECTED").length,39);
