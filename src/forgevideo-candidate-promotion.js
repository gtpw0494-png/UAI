...
export class ForgeVideoCandidatePromotion{
  constructor({root=process.cwd(),stateRoot=path.resolve("state"),python=process.env.PYTHON||"python3",runner=run,audit=null}={}){
    ...
    this.live=path.join(root,"model","checkpoints","forgevideo.pt");
    this.dir=path.join(stateRoot,"video-rollbacks");fs.mkdirSync(this.dir,{recursive:true});
  }
  status(){
    if(!fs.existsSync(this.live))return{state:"UNAVAILABLE",checkpointExists:false,liveCheckpoint:this.live,videoInference:false,message:"No promoted ForgeVideo checkpoint exists."};
    return{state:"CONFIGURED",checkpointExists:true,liveCheckpoint:this.live,sha256:sha256File(this.live),videoInference:false,message:"A promoted ForgeVideo checkpoint exists; runtime health must pass before CONNECTED is reported."};
  }
  async evaluate({candidate,dataset,minCosine=0.05,maxRelativeRegression=0.02}={}){
    ...
    if(!fs.existsSync(cand))return{state:"UNAVAILABLE",eligible:false,message:"ForgeVideo candidate checkpoint missing."};
    if(!fs.existsSync(data))return{state:"UNAVAILABLE",eligible:false,message:"ForgeVideo evaluation dataset missing."};
    const args=["model/eval/video_compare.py","--candidate",cand,"--dataset",data,"--min-cosine",String(minCosine),"--max-relative-regression",String(maxRelativeRegression)];
    if(fs.existsSync(this.live))args.push("--baseline",this.live);
    const raw=await this.runner(this.python,args,{cwd:this.root});
    const comparison=lastJson(raw.stdout)||{state:raw.state,message:raw.stderr||"ForgeVideo comparison produced no JSON."};
    ...
    this.audit?.append?.({type:"forgevideo.candidate.evaluated",state:out.state,eligible:out.eligible,candidateSha256:candidateSha,baselineSha256:baselineSha});
    return out;
  }
  snapshotBaseline(label="pre-promotion"){
    if(!fs.existsSync(this.live))return{state:"SUCCESS",snapshot:null,bootstrap:true};
    const id="video-rollback-"+crypto.randomUUID(),backup=path.join(this.dir,id+".pt");atomicCopy(this.live,backup);
  ...
  promote({candidate,evaluation,approved=false,approvalId=null}={}){
    if(!evaluation?.eligible)return{state:"BLOCKED",promoted:false,message:"ForgeVideo candidate has not passed evaluation."};
    ...
    const snapshot=this.snapshotBaseline("pre-forgevideo-promotion");if(snapshot.state!=="SUCCESS")return snapshot;
    ...
    this.audit?.append?.({type:"forgevideo.candidate.promoted",approvalId,candidateSha256:evaluation.candidate_sha256,liveSha256:liveSha,rollbackId:snapshot.snapshot?.id||null,bootstrap:out.bootstrap});
    return out;
  }
  rollback(id,{reason="video regression"}={}){
    if(!id)return{state:"BLOCKED",message:"A rollback snapshot ID is required."};
    const meta=path.join(this.dir,String(id)+".json");if(!fs.existsSync(meta))return{state:"FAILURE",message:"ForgeVideo rollback snapshot not found."};
    const record=JSON.parse(fs.readFileSync(meta,"utf8"));if(!fs.existsSync(record.backup))return{state:"FAILURE",message:"ForgeVideo rollback artifact missing."};
    if(sha256File(record.backup)!==record.sha256)return{state:"BLOCKED",message:"ForgeVideo rollback artifact integrity check failed."};
    atomicCopy(record.backup,this.live);const liveSha=sha256File(this.live);
    this.audit?.append?.({type:"forgevideo.rollback",rollbackId:id,reason,liveSha256:liveSha});
    return{state:"SUCCESS",rolled_back:true,rollback_id:id,reason,live_sha256:liveSha};
  }
}
export default ForgeVideoCandidatePromotion;
