...
export class SelfDevelopmentEngine{
  ...
  stage(proposalId){
    ...
    const npm=spawnSync("npm",["test"],{cwd:dir,encoding:"utf8",timeout:120000,env:{...process.env,IU_SELFDEV_SANDBOX:"1"}});checks.push({name:"repository-tests",state:npm.status===0?"SUCCESS":"FAILURE",output:(npm.stdout+"\n"+npm.stderr).slice(-5000)});
