...
export class KnowledgeVerifier {
  ...
  verifyGroup(items=[]) {
    ...
    for(const [claim_hash,group] of groups){
      ...
      facts.push({...primary,claim_hash,corroboration:independent.length,rights_corroboration:rightsIndependent.length,supporting_sources:valid.map(x=>({source_id:x.source_id,source_url:x.source_url,trust:x.source_trust,content_hash:x.content_hash,rights_eligible:x.rights_eligible,training_policy:x.training_policy})),verification:{verified,method:"independent-approved-source-corroboration",required_sources:this.minIndependentSources,independent_sources:independent.length,training_rights_verified:trainingEligible,rights_sources:rightsIndependent.length},training_eligible:trainingEligible});
    }
