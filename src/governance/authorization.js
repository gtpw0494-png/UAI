...
export function routeSecurity(method,path){
  ...
  if(/\/purge$|\/forget-source$|\/source\/rollback$|\/model\/train$|\/model\/tokenizer\/train$|\/model\/(?:promote|rollback)$|\/knowledge\/learning\/train$|\/knowledge\/model\/(?:promote|rollback)$|\/knowledge\/research\/schedule$|\/knowledge\/cloud\/recover$|\/selfdev\/promote$|\/develop\/apply$|\/fabrication\/job$|\/autonomy\/grant$|^\/api\/promotion\/|\/governance\/emergency\/release$|\/governance\/trusted-devices\/(?:enroll|revoke)$/.test(p))risk="high";
  ...
  if(p.startsWith("/api/web/")||p.startsWith("/api/provider/")||p.startsWith("/api/billing/")||p.startsWith("/api/oxford/")||p.startsWith("/api/fabrication/")||p.startsWith("/api/knowledge/research")||p==="/api/knowledge/cloud/recover"||p.endsWith("/execute"))external=true;
  if(p.startsWith("/api/billing/")||p.startsWith("/api/oxford/")||p.startsWith("/api/fabrication/")||p.startsWith("/api/provider/")||p==="/api/knowledge/cloud/recover")requiresCredential=true;
