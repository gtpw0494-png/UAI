...
const child=spawn(process.execPath,["server.js"],{cwd:repoRoot,env:{...process.env,PORT:String(port),IUV_STATE_DIR:stateRoot,IUV_DB_PATH:path.join(stateRoot,"data.sqlite3"),IUV_OBJECT_ROOT:path.join(stateRoot,"objects")},stdio:["ignore","pipe","pipe"]});
