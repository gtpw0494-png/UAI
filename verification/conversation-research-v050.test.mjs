...
const onechat=new OneChatRouter({conversation,webResearch,store,audit,explorative,knowledge,responseComposer:{compose({contributions}){const x=contributions.find(c=>c.agent==="conversation")?.result;return {message:x?.message||"none",mode:"native-conversation",modelUsed:true,evidence:null};}}});
