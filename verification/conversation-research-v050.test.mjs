...
const onechat=new OneChatRouter({conversation,webResearch,store,audit,explorative,responseComposer:{compose({contributions}){const x=contributions.find(c=>c.agent==="conversation")?.result;return {message:x?.message||"none",mode:"native-conversation",modelUsed:true,evidence:null};}}});
const out=await onechat.handle({chatId:"c2",message:"research the latest local-first evidence"});
assert.equal(out.state,"SUCCESS");assert.equal(out.evidenceEnvelope.metadata.researchRunId!=null,true);assert.equal(out.evidenceEnvelope.claims[0].support.length,2);assert.match(out.message,/\[S1\]/);
